...
typedef b32 (*ProperLexerFn)(Arena *arena, Token_List *list, void *state_ptr, u64 max);
struct ProperLexer {
    void *state;
    ProperLexerFn full_input_breaks;
};

function ProperLexer init_proper_lexer(Arena *arena, String_Const_u8 contents, bool is_lua) {
    if (is_lua) {
        Lex_State_Lua* state = push_array(arena, Lex_State_Lua, 1);
        lex_full_input_lua_init(state, contents);

        ProperLexer l;
        l.state = state;
        l.full_input_breaks = (ProperLexerFn) lex_full_input_lua_breaks;
        return l;
    } else {
        Lex_State_Cpp* state = push_array(arena, Lex_State_Cpp, 1);
        lex_full_input_cpp_init(state, contents);

        ProperLexer l;
        l.state = state;
        l.full_input_breaks = (ProperLexerFn) lex_full_input_cpp_breaks;
        return l;
    }
}
...
do_full_lex_async__inner(Async_Context *actx, Buffer_ID buffer_id){
    ...
    ProperLexer lex = init_proper_lexer(scratch, contents, *lang_id_ptr);
    for (;;){
        ProfileBlock(app, "async lex block");
        if (lex.full_input_breaks(scratch, &list, lex.state, limit_factor)){
            break;
        }
        if (async_check_canceled(actx)){
            canceled = true;
...
BUFFER_HOOK_SIG(default_begin_buffer){
    ...
    *eol_setting = setting;
    ...
    b32 *lang_id_ptr = scope_attachment(app, scope, buffer_language, b32);
    *lang_id_ptr = string_match(ext, SCu8("lua"));
...
BUFFER_EDIT_RANGE_SIG(default_buffer_edit_range){
    ...
    if (ptr != 0 && ptr->tokens != 0){
        ...
        if (token_index_resync_guess - token_index_first >= 4000){
            ...
            ProperLexer lex = init_proper_lexer(scratch, partial_text, *lang_id_ptr);
            lex.full_input_breaks(scratch, &relex_list, lex.state, max_u64);
