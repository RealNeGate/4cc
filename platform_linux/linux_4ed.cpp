...
internal Epoll_Kind epoll_tag_user_timer = EPOLL_USER_TIMER;
...
linux_thread_proc_start(void* arg) {
    ...
    // spall_auto_thread_init(pthread_self(), 2*1024*1024);
...
linux_epoll_process(struct epoll_event* events, int num_events) {
    ...
    for (int i = 0; i < num_events; ++i){
        ...
        switch (*tag){
            case EPOLL_X11: {
                do_step |= linux_handle_x11_events();
            } break;
...
main(int argc, char **argv){
    ...
    for (;;) {
        ...
        // there's really really weird behavior around epoll failing to catch all inputs
        if (XPending(linuxvars.dpy)) {
            events[0].events = EPOLLIN | EPOLLET;
            events[0].data.ptr = &epoll_tag_x11;
            num_events++;
        }

        int r = epoll_wait(linuxvars.epoll, &events[num_events], ArrayCount(events) - num_events, num_events ? 0 : -1);
        system_mutex_acquire(linuxvars.global_frame_mutex);
