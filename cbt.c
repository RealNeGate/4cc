    ...
    if (path != NULL) {
        ...
        for (size_t i = 0; i < pt->og_buffer_size; i++) {
            if (pt->og_buffer[i] == '\n') {
                ...
                pt_insert(&pt->line_starts, last_line, (PT_Val){ 0, 0, i - last_line });
                ...
                for (size_t j = 0; j < i; j++) {
                    if (pt->og_buffer[j] == '\n') {
                        ...
                        assert(piece->length == j - last_line2);
                        last_line2 = j;
                    }
                }
                #endif
...
void pt_replace_range(PieceTable* pt, uint64_t start, uint64_t end, size_t length, const char* text) {
    ...
    ////////////////////////////////
    // Delete range of [start, end), shifts right down
    ////////////////////////////////
    if (start != end && pt->line_starts.root != NULL && pt->line_starts.root->count > 0) {
        PT_Cursor left = pt_lookup(&pt->line_starts, start ? start - 1 : 0);
        // pt_dump(pt->line_starts.root, 0, 0);
        ...
        skip2:;
        if (start == 0) {
            pt_insert(&pt->line_starts, 0, (PT_Val){ 0, 0, to_preserve });
        } else {
            // Find the start-1 entry
            left = pt_lookup(&pt->line_starts, start ? start - 1 : 0);
            PT_Val* piece = pt_get_val(left);
    ...
    if (pt->piece_tables.root != NULL && pt->piece_tables.root->count > 0) {
        ...
        if (left_end == start && start == end && pt->added_buffer_used == left_v->offset + left_v->length) {
        ...
        if (0) {
            int i = 0;
            for (; i < start; i++) { printf(" "); }
            if (end == 0) {
                printf("*");
            } else {
                for (; i < end; i++) { printf("_"); }
            }
            printf("\n");
        }
