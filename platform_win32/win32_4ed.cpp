...
win32_proc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
    ...
    switch (uMsg){
        ...
        case WM_DPICHANGED:
        {
            win32vars.screen_scale_factor = HIWORD(wParam) / 96.0f;
            log_os(" detected dpi %f\n", win32vars.screen_scale_factor);

            RECT* const new_wnd = (RECT*) lParam;
            SetWindowPos(hwnd,
                NULL,
                new_wnd->left,
                new_wnd->top,
                new_wnd->right - new_wnd->left,
                new_wnd->bottom - new_wnd->top,
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow){
    ...
    SetProcessDpiAwarenessContext(DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE);
    {
        ...
        HMONITOR primary_monitor = MonitorFromPoint(ptZero, MONITOR_DEFAULTTOPRIMARY);

        UINT dpi_x, dpi_y;
        GetDpiForMonitor(primary_monitor, MDT_EFFECTIVE_DPI, &dpi_x, &dpi_y);

        win32vars.screen_scale_factor = dpi_x / 96.0f;
        log_os(" detected dpi %f\n", win32vars.screen_scale_factor);
    }
